...
def main():
    ...
    try:
        print("\n=== 텍스트 파일 처리 ===")
        text_content = process_text_file("과제 테스트용.txt")
        print(f"텍스트 파일 내용: {text_content}")

        print("\n=== JSON 파일 처리 ===")
        json_content = process_json_file("과제 테스트용.json")
        print(f"JSON 파일 내용: {json_content}")

        print("\n=== CSV 파일 처리 ===")
        csv_content = process_csv_file("과제 테스트용.csv")
    ...
    # 1. FileNotFoundError 발생시키기
    try:
        process_text_file("nonexistent.txt")
    except FileNotFoundError as e:
        print(f"예외 처리됨: {e}")

    # 2. InvalidFileFormatError 발생시키기
    with open("invalid_format.json", "w") as f:
        f.write("This is not a valid JSON")
    try:
        process_json_file("invalid_format.json")
    except InvalidFileFormatError as e:
        print(f"예외 처리됨: {e}")
    finally:
        os.remove("invalid_format.json")
        
    # 3. PermissionError 발생시키기 (시스템에 따라 권한 오류가 발생하지 않을 수 있음)
